...
for i=1:nSymbol
    if i==nSymbol
        ...
        for j=1:ceil(size(temp,2)/Nu)
            resource_grid(i,8:8+size(temp,2)-1,j) = temp;
        end
    else
        temp = code(1+(i-1)*Nu*numBand:i*Nu*numBand);
        for j=1:numBand
            resource_grid(i,8:8+Nu-1,j) = temp(1+(j-1)*Nu:1+j*Nu-1);
        end
    end
    ...
    for j=1:numBand
        rawSignal(j,(i-1)*NFFT+1:i*NFFT) = ifft(resource_grid(i,:,j),NFFT); % 对资源网格进行IFFT
    end
end
