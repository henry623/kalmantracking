...
function [kf_output] = standard_kf(kf, z, dt)
    ...
    % 码相位误差 += 码频率误差 * dt
    % 载波相位误差 += 载波频率误差 * dt
    kf.F = [1 dt 0 0;   % 码相位误差 = 码相位误差 + 码频率误差*dt
            0  1 0 0;   % 码频率误差保持不变
            0  0 1 dt;  % 载波相位误差 = 载波相位误差 + 载波频率误差*dt
            0  0 0  1]; % 载波频率误差保持不变
...
function [kf_output] = extended_kf(kf, z, dt)
    ...
    h = @(x) [x(1) + 0.1*sin(x(2)); x(3) + 0.1*sin(x(4))];  % 非线性测量函数示例
    
    % 计算测量函数在当前状态估计处的雅可比矩阵
    H = [1 0.1*cos(kf.x(2)) 0 0;
         0 0 1 0.1*cos(kf.x(4))];
...
function [kf_output] = unscented_kf(kf, z, dt)
    ...
    % 定义非线性测量函数
    % 这里使用一个简单的非线性函数作为示例
    h = @(x) [x(1) + 0.1*sin(x(2)); x(3) + 0.1*sin(x(4))];
